...
(function () {
    ...
    function replaceW(w) {
        if (w.endsWith("w")) {
            return parseFloat(w.replace("w", "")) * 10000
        }
        return parseInt(w)
    }
    ...
    scrollButton.onclick = function () {
        ...
        const interval = setInterval(() => {
            ...
            if (result) {
                ...
                const csv_header = "date,title,likes,views\n"
                const csv_lines = data.map((item) => `"${item.date}","${item.title}",${replaceW(item.likes)},${replaceW(item.views)}`);
                download(setDate + "抖音数据.csv", csv_header + csv_lines.join('\n'));
            }
